...
//#endregion
//#region src/core/composables/useToast.ts
var message = /* @__PURE__ */ ref("");
var tone = /* @__PURE__ */ ref("success");
var timer;
function useToast() {
...
//#endregion
//#region src/core/composables/useConfetti.ts
/** 彩帶顏色盤：紙屑依索引循環套用這 7 色 */
var COLS = [
	"#ff6b6b",
	"#ffb13c",
	"#36c5b0",
...
/**
* 命令式 confetti 效果 composable。
*
* 動態建立一個 `.o-confetti` 容器並 append 數十顆彩色紙屑到 `document.body`，
* 播放掉落動畫後於 5.2 秒自動清除。與模組級單例的 composable 不同，本函式
* 每次呼叫各自持有一份 DOM 狀態（`el`／`timer`），彼此獨立、互不共用。
*
* @returns 提供 `launch`（觸發彩帶）與 `clear`（立即清除）兩個方法
*/
function useConfetti() {
	/** 目前掛載於 body 的彩帶容器；未啟動時為 null */
	let el = null;
	/** 自動清除用的 setTimeout 計時器 id */
	let timer;
	/**
	* 觸發一次彩帶。會先呼叫 {@link clear} 清掉前一輪，再產生 `count` 顆紙屑，
	* 每顆隨機決定大小、形狀（圓形或長條）、顏色與動畫時長／延遲。
	*
	* @param count 紙屑數量，預設 80 顆
	*/
	function launch(count = 80) {
		clear();
		el = document.createElement("div");
		el.className = "o-confetti";
		for (let i = 0; i < count; i++) {
			const sz = 7 + Math.random() * 11;
			const round = Math.random() > .5;
			const p = document.createElement("span");
			p.style.cssText = "position:absolute;top:-30px;left:" + Math.random() * 100 + "%;width:" + sz + "px;height:" + (round ? sz : sz * .5) + "px;background:" + COLS[i % COLS.length] + ";border-radius:" + (round ? "50%" : "2px") + ";opacity:.95;animation:confettiFall " + (2.4 + Math.random() * 2).toFixed(2) + "s linear " + (Math.random() * .6).toFixed(2) + "s forwards;";
			el.appendChild(p);
		}
		document.body.appendChild(el);
		timer = window.setTimeout(clear, 5200);
	}
	/**
	* 立即移除彩帶容器並取消自動清除計時器，可安全重複呼叫。
	* 在 {@link launch} 開頭與元件卸載時皆應呼叫，避免殘留 DOM 與計時器。
	*/
	function clear() {
		if (el) {
			el.remove();
...
//#endregion
//#region src/core/composables/useLuckyDraw.ts
/** 使用者輸入的參加名單原文（以換行或逗號分隔） */
var namesText = /* @__PURE__ */ ref("");
/** 獎項清單，預設一個「得獎」1 名 */
...
/** 滾動階段畫面上快速變換的名字 */
var rollingName = /* @__PURE__ */ ref("");
/** 本輪各獎項的得獎結果（動畫開始前即已定案） */
...
var revealedGroups = computed(() => {
	...
	return groups.value.map((g) => {
		const show = Math.max(0, Math.min(g.winners.length, remaining));
		remaining -= g.winners.length;
		return {
			...g,
			revealed: g.winners.slice(0, show)
		};
	});
});
/**
...
* @returns 對應的色碼字串
*/
function prizeColor(i) {
	return PALETTE[i % PALETTE.length] ?? PALETTE[0];
}
/**
* 從有效名單隨機取一個名字（供滾動動畫與可重複模式抽獎使用）。
* @returns 隨機名字；名單為空時回傳空字串
*/
function randomName() {
	return pool.value[Math.floor(Math.random() * pool.value.length)] ?? "";
}
/**
* 匯入文字檔並附加到現有名單後方，完成後跳出 toast 提示。
* @param file 使用者選取的文字檔
*/
function importFile(file) {
	const r = new FileReader();
	r.onload = () => {
		const cur = namesText.value.trim();
		namesText.value = (cur ? cur + "\n" : "") + String(r.result).trim();
		toast(`已匯入「${file.name}」`);
	};
	r.readAsText(file);
}
...
/** 新增一個獎項（名額預設 1，自動編號並指派自增 id） */
function addPrize() {
	prizes.value.push({
		id: pid++,
		name: "獎項 " + (prizes.value.length + 1),
		count: 1
	});
}
/**
* 移除指定獎項；至少保留一個獎項，若只剩一個則不動作。
* @param id 要移除的獎項 id
*/
function removePrize(id) {
	if (prizes.value.length > 1) prizes.value = prizes.value.filter((p) => p.id !== id);
}
/**
...
/** 清除滾動與揭曉的計時器，避免殘留的 timer 繼續觸發 */
function clearTimers() {
	clearInterval(rollTimer);
	clearTimeout(revealTimer);
}
/**
* 抽獎進入點：驗證名單／名額後定案結果，切到 rolling 階段並啟動滾動動畫，
* 1.8s 後交棒給 {@link beginReveal}。抽獎中重複呼叫會被忽略。
*/
function runDraw() {
...
function beginReveal() {
	...
	const step = () => {
		...
		revealedCount.value++;
		revealTimer = window.setTimeout(step, 360);
	};
	revealTimer = window.setTimeout(step, 280);
}
/** 將得獎名單整理成文字並複製到剪貼簿（含 fallback，皆會跳 toast 提示） */
function copy() {
	if (!groups.value.length) {
		toast("還沒有抽獎結果");
		return;
	}
	const text = groups.value.map((g) => `${g.name}（${g.winners.length} 名）：${g.winners.join("、") || "從缺"}`).join("\n");
	if (navigator.clipboard?.writeText) navigator.clipboard.writeText(text).then(() => toast("已複製得獎名單")).catch(() => toast("已複製得獎名單"));
	else toast("已複製得獎名單");
}
/**
* 抽獎機核心 composable，匯出全 app 共用的模組級單例狀態與操作方法。
* 由於狀態宣告於模組頂層，任何元件呼叫本函式取得的都是同一組 reactive 參考。
*
* @returns 狀態（state）、衍生值（derived）與操作方法（methods）
*/
...
//#endregion
//#region src/views/components/atoms/AEyebrow.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$14 = { class: "a-eyebrow" };
var AEyebrow_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	__name: "AEyebrow",
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$14, [renderSlot(_ctx.$slots, "default", {}, void 0, true)]);
		};
	}
});
//#endregion
...
//#endregion
//#region src/views/components/atoms/AEyebrow.vue
var AEyebrow_default = /*#__PURE__*/ _plugin_vue_export_helper_default(AEyebrow_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-21e38fde"]]);
//#endregion
//#region src/views/components/organisms/OHero.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$13 = { class: "o-hero" };
//#endregion
//#region src/views/components/organisms/OHero.vue
var OHero_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "OHero",
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("header", _hoisted_1$13, [
				createVNode(AEyebrow_default, null, {
					default: withCtx(() => [..._cache[0] || (_cache[0] = [createTextVNode("Lucky Draw", -1)])]),
					_: 1
				}),
				_cache[1] || (_cache[1] = createBaseVNode("h1", { class: "o-hero__title" }, [createTextVNode("抽獎機"), createBaseVNode("span", { class: "o-hero__dot" }, ".")], -1)),
				_cache[2] || (_cache[2] = createBaseVNode("p", { class: "o-hero__sub" }, " 貼上或匯入名單，設定獎項與名額，按下按鈕公平抽出得獎者。所有運算都在你的瀏覽器裡完成。 ", -1))
			]);
		};
	}
}), [["__scopeId", "data-v-55a724fc"]]);
//#endregion
//#region src/views/components/molecules/MCard.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$12 = { class: "m-card" };
//#endregion
//#region src/views/components/molecules/MCard.vue
var MCard_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "MCard",
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("section", _hoisted_1$12, [renderSlot(_ctx.$slots, "default", {}, void 0, true)]);
		};
	}
}), [["__scopeId", "data-v-6edbd30c"]]);
//#endregion
//#region src/views/components/atoms/ANumBadge.vue
var ANumBadge_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "ANumBadge",
	props: { color: {} },
	setup(__props) {
		return (_ctx, _cache) => {
...
}), [["__scopeId", "data-v-244d3adf"]]);
//#endregion
//#region src/views/components/molecules/MSectionHeader.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$11 = { class: "m-card__head" };
var _hoisted_2$7 = { class: "m-card__title" };
//#endregion
//#region src/views/components/molecules/MSectionHeader.vue
var MSectionHeader_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "MSectionHeader",
	props: {
		num: {},
		color: {},
		title: {}
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$11, [createBaseVNode("h2", _hoisted_2$7, [createVNode(ANumBadge_default, { color: __props.color }, {
				default: withCtx(() => [createTextVNode(toDisplayString(__props.num), 1)]),
				_: 1
			}, 8, ["color"]), createTextVNode(toDisplayString(__props.title), 1)]), renderSlot(_ctx.$slots, "tail", {}, void 0, true)]);
		};
	}
}), [["__scopeId", "data-v-0656ddc1"]]);
//#endregion
//#region src/views/components/atoms/ABadge.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$10 = { class: "a-countPill" };
//#endregion
//#region src/views/components/atoms/ABadge.vue
var ABadge_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "ABadge",
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("span", _hoisted_1$10, [renderSlot(_ctx.$slots, "default")]);
		};
	}
}), [["__scopeId", "data-v-16b7314f"]]);
//#endregion
//#region src/views/components/atoms/ATextArea.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$9 = ["value", "placeholder"];
//#endregion
//#region src/views/components/atoms/ATextArea.vue
var ATextArea_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "ATextArea",
	props: {
		modelValue: {},
		placeholder: {}
...
}), [["__scopeId", "data-v-8e1211ae"]]);
//#endregion
//#region src/views/components/atoms/AButton.vue
var AButton_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "AButton",
	props: {
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("button", {
				type: "button",
				class: normalizeClass(["a-button", ["-" + __props.variant, { "-block": __props.block }]])
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(MCard_default, null, {
				default: withCtx(() => [
					createVNode(MSectionHeader_default, {
						num: 1,
						color: "var(--prize-2)",
						title: "參加名單"
					}, {
						tail: withCtx(() => [createVNode(ABadge_default, null, {
							default: withCtx(() => [
								_cache[4] || (_cache[4] = createTextVNode("有效 ", -1)),
								createBaseVNode("b", null, toDisplayString(unref(validCount)), 1),
					...
					createVNode(ATextArea_default, {
						modelValue: unref(namesText),
						"onUpdate:modelValue": _cache[0] || (_cache[0] = ($event) => /* @__PURE__ */ isRef(namesText) ? namesText.value = $event : null),
						placeholder: namesPlaceholder
					}, null, 8, ["modelValue"]),
					createBaseVNode("div", _hoisted_1$7, [
						createVNode(AButton_default, {
							variant: "ghost",
							onClick: _cache[1] || (_cache[1] = ($event) => fileInput.value?.click())
						}, {
							default: withCtx(() => [..._cache[6] || (_cache[6] = [createTextVNode("＋ 匯入檔案", -1)])]),
							_: 1
						}),
						createBaseVNode("input", {
							ref_key: "fileInput",
						...
						createVNode(AButton_default, {
							variant: "danger",
							onClick: unref(clearNames)
						}, {
							default: withCtx(() => [..._cache[7] || (_cache[7] = [createTextVNode("清空名單", -1)])]),
							_: 1
						}, 8, ["onClick"])
					]),
...
//#endregion
//#region src/views/components/molecules/MNumberStepper.vue
var MNumberStepper_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "MNumberStepper",
	props: {
		modelValue: {},
		min: { default: 1 }
	},
	...
	setup(__props, { emit: __emit }) {
		...
		const emit = __emit;
		function step(d) {
			emit("update:modelValue", Math.max(props.min, (Number(props.modelValue) || 1) + d));
		}
		function onInput(e) {
			const v = parseInt(e.target.value, 10);
			emit("update:modelValue", isNaN(v) ? props.min : Math.max(props.min, v));
		}
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$6, [
				createBaseVNode("button", {
					class: "m-numberStepper__btn",
					type: "button",
					disabled: __props.modelValue <= __props.min,
					"aria-label": "減少名額",
					onClick: _cache[0] || (_cache[0] = ($event) => step(-1))
				}, "－", 8, _hoisted_2$4),
				createBaseVNode("input", {
					class: "m-numberStepper__value",
					type: "text",
					inputmode: "numeric",
					value: __props.modelValue,
					"aria-label": "名額",
...
}), [["__scopeId", "data-v-67dd20b2"]]);
//#endregion
//#region src/views/components/molecules/MPrizeRow.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$5 = { class: "m-prizeRow" };
var _hoisted_2$3 = ["value"];
//#endregion
//#region src/views/components/molecules/MPrizeRow.vue
var MPrizeRow_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "MPrizeRow",
	props: {
		prize: {},
		index: {},
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$5, [
				createBaseVNode("span", {
					class: "m-prizeRow__marker",
					style: normalizeStyle({ background: __props.color })
				}, toDisplayString(__props.index + 1), 5),
				createBaseVNode("input", {
					class: "m-prizeRow__name",
					type: "text",
					value: __props.prize.name,
					placeholder: "獎項名稱",
					onInput: _cache[0] || (_cache[0] = ($event) => emit("update:name", $event.target.value))
				}, null, 40, _hoisted_2$3),
				createVNode(MNumberStepper_default, {
					"model-value": __props.prize.count,
					"onUpdate:modelValue": _cache[1] || (_cache[1] = (v) => emit("update:count", v))
				}, null, 8, ["model-value"]),
				__props.showRemove ? (openBlock(), createElementBlock("button", {
					key: 0,
					class: "m-prizeRow__remove",
					type: "button",
					title: "刪除",
...
//#endregion
//#region src/views/components/organisms/OPrizesPanel.vue
			...
			return openBlock(), createBlock(MCard_default, null, {
				default: withCtx(() => [
					createVNode(MSectionHeader_default, {
						...
						tail: withCtx(() => [createVNode(ABadge_default, null, {
							default: withCtx(() => [
								_cache[0] || (_cache[0] = createTextVNode("共 ", -1)),
								createBaseVNode("b", null, toDisplayString(unref(need)), 1),
								_cache[1] || (_cache[1] = createTextVNode(" 個名額", -1))
							]),
							_: 1
						})]),
						_: 1
					}),
					createBaseVNode("div", _hoisted_1$4, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(prizes), (p, i) => {
						return openBlock(), createBlock(MPrizeRow_default, {
							key: p.id,
							prize: p,
							index: i,
							color: unref(prizeColor)(i),
							"show-remove": unref(prizes).length > 1,
							"onUpdate:name": (v) => p.name = v,
							"onUpdate:count": (v) => p.count = v,
							onRemove: ($event) => unref(removePrize)(p.id)
						}, null, 8, [
							"prize",
							"index",
							"color",
					...
					createVNode(AButton_default, {
						...
						variant: "add",
						block: "",
						onClick: unref(addPrize)
					}, {
						default: withCtx(() => [..._cache[2] || (_cache[2] = [createTextVNode("＋ 新增獎項", -1)])]),
						_: 1
					}, 8, ["onClick"]),
...
var _hoisted_1$3 = {
	key: 0,
	class: "a-wchip is-empty"
};
//#endregion
//#region src/views/components/atoms/AWinnerChip.vue
var AWinnerChip_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "AWinnerChip",
	props: {
		name: {},
		color: {},
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return __props.empty ? (openBlock(), createElementBlock("span", _hoisted_1$3, "從缺")) : (openBlock(), createElementBlock("span", {
				key: 1,
				class: "a-wchip",
				style: normalizeStyle({ "--rc": __props.color })
			}, [_cache[0] || (_cache[0] = createBaseVNode("span", { class: "a-wchip__star" }, "★", -1)), createTextVNode(toDisplayString(__props.name), 1)], 4));
		};
	}
}), [["__scopeId", "data-v-44de6977"]]);
//#endregion
//#region src/views/components/molecules/MResultCard.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$2 = { class: "m-resultCard__head" };
var _hoisted_2$2 = { class: "m-resultCard__name" };
var _hoisted_3$2 = { class: "m-resultCard__count" };
var _hoisted_4$2 = { class: "m-resultCard__winners" };
//#endregion
//#region src/views/components/molecules/MResultCard.vue
var MResultCard_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "MResultCard",
	props: {
		name: {},
		color: {},
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				class: "m-resultCard",
				style: normalizeStyle({ "--rc": __props.color })
			}, [createBaseVNode("div", _hoisted_1$2, [createBaseVNode("span", _hoisted_2$2, toDisplayString(__props.name), 1), createBaseVNode("span", _hoisted_3$2, toDisplayString(__props.count) + " 名", 1)]), createBaseVNode("div", _hoisted_4$2, [(openBlock(true), createElementBlock(Fragment, null, renderList(__props.revealed, (w, i) => {
				return openBlock(), createBlock(AWinnerChip_default, {
					key: i,
					name: w,
					color: __props.color
				}, null, 8, ["name", "color"]);
			}), 128)), __props.empty ? (openBlock(), createBlock(AWinnerChip_default, {
				key: 0,
				empty: ""
			})) : createCommentVNode("", true)])], 4);
		};
	}
}), [["__scopeId", "data-v-ec61940f"]]);
//#endregion
//#region src/views/components/organisms/OStage.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1$1 = {
	key: 0,
	class: "o-stage"
};
var _hoisted_2$1 = {
	key: 0,
	class: "o-stage__rolling"
};
var _hoisted_3$1 = { class: "o-stage__pill" };
var _hoisted_4$1 = { class: "o-stage__name" };
var _hoisted_5$1 = { key: 1 };
var _hoisted_6 = { class: "o-stage__head" };
var _hoisted_7 = {
	key: 0,
	class: "o-stage__tools"
};
var _hoisted_8 = { class: "o-stage__grid" };
//#endregion
//#region src/views/components/organisms/OStage.vue
...
var AToast_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				class: normalizeClass(["a-toast", [{ "is-show": __props.show }, "-" + (__props.tone || "success")]]),
				role: "status"
			}, toDisplayString(__props.message), 3);
		};
	}
}), [["__scopeId", "data-v-21bf1de9"]]);
//#endregion
//#region src/views/LuckyDrawView.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1 = { class: "t-draw" };
var _hoisted_2 = { class: "t-draw__grid" };
var _hoisted_3 = { class: "t-draw__action" };
var _hoisted_4 = { class: "t-draw__hint" };
var _hoisted_5 = { class: "t-draw__stage" };
//#endregion
//#region src/views/LuckyDrawView.vue
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock(Fragment, null, [createBaseVNode("div", _hoisted_1, [
				createVNode(OHero_default),
				createBaseVNode("div", _hoisted_2, [createVNode(ONamesPanel_default), createVNode(OPrizesPanel_default)]),
...
var router = createRouter({
	history: createWebHistory("/lucky-draw/"),
	routes: [{
		path: "/",
		name: "lucky-draw",
		component: LuckyDrawView_default
	}]
});
//#endregion
//#region src/main.ts
